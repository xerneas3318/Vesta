...
def describe_video(video_path: str, n: int, t: int, prompt: str):
    ...
    return gallery, all_captions, first_mosaic, all_captions


def summarize_mosaic_answers(first_mosaic: Image.Image | None, mosaic_answers: str) -> str:
    if first_mosaic is None:
        raise gr.Error("Please run Analyze Video first so the first mosaic is available.")
    if not mosaic_answers or not mosaic_answers.strip():
        raise gr.Error("No mosaic answers available. Run Analyze Video first.")
...
def build_app() -> gr.Blocks:
    with gr.Blocks(title="Video Understanding with Temporal n x n Mosaics") as demo:
        ...
        summarize_btn = gr.Button("Summarize Mosaic Answers")
        ...
        first_mosaic_state = gr.State()
        mosaic_answers_state = gr.State()

        run_btn.click(
            fn=describe_video,
            inputs=[video, n, t, prompt],
            outputs=[mosaic_gallery, caption_out, first_mosaic_state, mosaic_answers_state],
        )
        summarize_btn.click(
            fn=summarize_mosaic_answers,
            inputs=[first_mosaic_state, mosaic_answers_state],
            outputs=[final_summary_out],
        )
